...
class Solution:
    def encode(self, strs: list[str]) -> str:
        """Encode a list of strings into a single string using length-prefixed chunking.

        Each string is formatted as `<length>#<string>`, which unambiguously demarcates
        boundaries regardless of whether strings contain `#`, whitespace, digits, or colons.

        Args:
            strs: List of strings to encode.

        Returns:
            A single encoded string containing all serialized items.
        """
        # Build individual length-prefixed string chunks in linear time.
        # Joining list chunks avoids quadratic string concatenation overhead.
        chunks: list[str] = []
        for s in strs:
    ...
    def decode(self, s: str) -> list[str]:
        """Decode a single length-prefixed string back into the original list of strings.

        Iteratively parses the integer length preceding each `#` delimiter, extracting
        the exact slice of characters specified by that length.

        Args:
            s: Encoded string produced by `encode`.

        Returns:
            The reconstructed list of original strings.
        """
        # Guard clause: an empty encoded string corresponds to an empty input list.
        # This occurs because encode([]) yields zero chunks, producing "".
        if not s:
            return []
        ...
        while i < n:
            # Locate the '#' delimiter terminating the current chunk's length prefix.
            # Scanning from index i guarantees we locate the delimiter for the current chunk.
            delimiter_idx = s.find("#", i)
            length = int(s[i:delimiter_idx])
            start = delimiter_idx + 1
            end = start + length

            # Slice the exact string payload as indicated by the parsed length.
            # Advances the scanning index past the extracted chunk for the next iteration.
