...
class Solution:
    def twoSum(self, nums: list[int], target: int) -> list[int]:
        ...
        Uses a single-pass hash map to locate the complementary value needed
        to sum to target, achieving O(n) time and O(n) space complexity.
        ...
        Raises:
            ValueError: If no valid two sum pair exists in nums.
        """
        # Map number values to their corresponding index in nums.
        # This provides O(1) average lookup time for complement checks.
        seen: dict[int, int] = {}

        # Scan each element in a single pass while calculating complement needed.
        for idx, num in enumerate(nums):
            complement = target - num

            # If the complement was previously observed, a valid pair has been found.
            # Returning immediately satisfies the constraint of exactly one solution.
            if complement in seen:
                return [seen[complement], idx]

            # Register current element index after complement lookup.
            # Post-lookup insertion prevents pairing an element with itself.
            seen[num] = idx

        # Defensive fallback if input violates problem constraint.
