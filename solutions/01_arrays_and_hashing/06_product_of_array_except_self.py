...
class Solution:
    def productExceptSelf(self, nums: list[int]) -> list[int]:
        ...
        Computes prefix products in a forward pass and accumulates postfix products in a
        backward pass directly inside the output array in O(n) time and O(1) auxiliary space
        ...
        # Initialize the output array with neutral multiplicative identity (1).
        # Per problem constraints, the returned list does not count as auxiliary space.
        res = [1] * n

        # First pass: compute running prefix products from left to right.
        # At index i, res[i] holds the product of all elements strictly to the left (0 .. i-1).
        prefix = 1
        for i in range(n):
            res[i] = prefix
            prefix *= nums[i]

        # Second pass: accumulate running postfix products from right to left.
        # Multiplying res[i] by postfix incorporates the product of all elements strictly to the right.
        postfix = 1
